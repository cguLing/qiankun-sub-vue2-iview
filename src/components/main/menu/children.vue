<template>
    <div :class="'menu-level-'+level">
        <template v-for="item in menuList" >
            <template>
                <Menu-item v-if="!item.children" v-show="!item.hide" :name="item.name" :key="item.name" @click.native="selectToRouterMain(item.name, item.target)">
                    <span>
                ...
                <Submenu v-if="item.children" v-show="!item.hide" :name="item.name" :key="item.name" @click.native="selectToRouterMain(item.name, item.target)">
                    <template slot="title">
                        <span>
                            <!-- <myIcon v-if="level==0" :type="item.icon||'icon-defaultLogo'"/> -->
                            {{item.meta['title']?item.meta.title:item.name}}
